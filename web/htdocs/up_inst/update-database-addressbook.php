...
while($daten=mysql_fetch_assoc($result))
 {
	$sql_query_addr=sprintf("INSERT INTO addressbook VALUES('','%s','%s','%s','%s','%s','%s','%s')",
	...
	{ //FOR
		...
		if ($daten[$tele]!= "99")
		{
			...
			$sql_query_phonenumber="INSERT INTO phonenumbers VALUES('','$last_id','$daten[$tele]', '1', '$prefix')";
	...
	if ($daten['fax']!= "99" && $daten['fax']!="")
	{
		...
		$sql_query_phonenumber="INSERT INTO phonenumbers VALUES('','$last_id','$daten[fax]', '3', '$prefix')";
		$res=mysql_query($sql_query_phonenumber);
		if (!$res)
		{
			echo "<br>Error in Insert new data to the phonenumber table<br>";
